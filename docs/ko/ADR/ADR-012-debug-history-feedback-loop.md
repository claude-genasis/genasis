...
메인테이너(리포 소유자)가 Claude Code와 `/debug-review` 스킬로
축적된 패치를 처리한다:

1. **트리거**: 수동 또는 스케줄 (예: 주간)
2. **입력**: `debug-history/patches/`의 모든 미해결 패치
3. **출력**: 템플릿 수정이 포함된 자동 생성 브랜치 + PR
4. **검토**: 메인테이너가 자동 PR 검토, 승인 또는 반복
5. **루프 종료**: 머지된 수정 → 패치를 resolved 태그 → 다음 릴리즈에
   개선 포함

이는 다음을 의미한다:
- **기여자로부터의 위험 제로** — 데이터만 추가 가능
- **최대 신호 추출** — 모든 패치를 기계 분석
- **최소 메인테이너 노력** — Claude Code가 무거운 작업 수행;
  메인테이너는 자동 PR 승인/거부만
