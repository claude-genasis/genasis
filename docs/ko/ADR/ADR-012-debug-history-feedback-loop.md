...
### 8. 기여 거버넌스 — Data-Only PR 모델

외부 기여자(포크 사용자 및 협력자)는 엄격한 책임 분리를 통해
debug-history와 상호작용한다:
...
│                                                                │
│  허용:                                                         │
│    • debug-history/patches/에 PR (patch.json만 제출)           │
│    • 패치에 user_comment로 맥락 설명 추가                      │
│    • [debug-history] 태그 이슈 오픈                            │
│                                                                │
│  불허:                                                         │
│    • 디버그 데이터 기반으로 템플릿(.tera) 수정                 │
│    • 디버그 데이터 기반으로 overlay 소스 파일 수정             │
│    • analysis/ 또는 clusters.md 수정                           │
│    • 디버그 패치에 직접 근거해 genasis 코드 변경               │
└────────────────────────────────────────────────────────────────┘
          │
          │  patch.json 축적
          ▼
┌────────────────────────────────────────────────────────────────┐
│  메인테이너 (Claude Code를 통한 자동개발)                      │
│                                                                │
│  1. /debug-review가 축적된 패치 읽기                           │
│  2. Claude Code가 클러스터링, 분석, 수정 제안                  │
│  3. 메인테이너가 자동 생성 PR 검토                             │
│  4. 머지된 수정이 다음 genasis 릴리즈에 포함                   │
│  5. 해결된 패치를 index.jsonl에 태그                           │
└────────────────────────────────────────────────────────────────┘
```

#### 이 분리가 중요한 이유

| 기여자가 직접 코드 수정 시 위험 | Data-only 모델로 완화 |
|---|---|
| 악의적 템플릿 주입 (예: 모든 genasis 사용자에게 실행되는 overlay hook에 명령 주입) | 기여자는 실행 가능 템플릿을 절대 건드리지 않음 — 변경 내용을 설명하는 비활성 JSON만 제출 |
| 일관성 없는 수정 품질 (기여자 케이스는 고치지만 다른 사용자 깨뜨림) | 메인테이너의 Claude Code가 모든 패치를 보고 수정이 다른 사용자 드리프트와 충돌하지 않는지 검증 |
| 리뷰 부담 폭발 (많은 기여자의 템플릿 로직 PR 검토) | `patch.json` 검토는 간단 — 알려진 스키마의 구조화된 데이터 |
| 공급망 공격 표면 | debug-history/patches/는 순수 데이터; CI가 아무것도 실행하지 않고 스키마 검증 가능 |

#### CI 강제

```yaml
# .github/workflows/debug-history-pr.yml
# debug-history/ 변경하는 PR 검증
- debug-history/patches/*.patch.json에 매칭되는 파일만 허용
- JSON 스키마 검증 (schema_version, 필수 필드)
- 실행 가능 콘텐츠 없음 (shebang, diff 비맥락 백틱 포함 시 거부)
- 자동 라벨: [debug-history]
- 자동 할당: 메인테이너
# API 키 없음, 시크릿 없음 — 순수 데이터 검증만
```

#### 메인테이너의 자동개발 흐름

메인테이너(리포 소유자)가 **로컬 Claude Code** (CI 아님, API 아님) 와
`/debug-review` 스킬로 축적된 패치를 처리한다:

1. **트리거**: 메인테이너가 genasis 리포에서 로컬 Claude Code 열기
2. **입력**: `debug-history/patches/`의 모든 미해결 패치
3. **처리**: `/debug-review` 스킬이 패치 읽기 → 템플릿 Edit 제안
4. **검토**: 메인테이너가 세션 내에서 변경 확인, 수락 또는 반복
5. **커밋**: 메인테이너가 수정 커밋 + push
6. **루프 종료**: 머지된 수정 → 패치를 resolved 태그 → 다음 릴리즈

**API 키 미사용. 클라우드 자동화 없음.** 메인테이너의 Claude Code Pro
구독만으로 충분하다. 이는 다음을 보장한다:
- **기여자로부터의 위험 제로** — 데이터만 추가 가능
- **API 비용 제로** — Claude Code Pro가 모든 분석 담당
- **완전한 메인테이너 제어** — 모든 변경을 세션 내 검토
- **최대 신호 추출** — 모든 패치를 기계 분석
- **감사 추적** — 모든 수정이 동기를 제공한 패치로 역추적 가능
