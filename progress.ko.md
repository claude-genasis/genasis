...
- 2026-05-05: **Phase E (Dynamic Agents Catalog) 착수** — ADR-011 채택. `include_dir!()` 제거, GitHub Releases tarball 기반 런타임 fetch 모델로 전환. agents/ 디렉토리에 9역할 best-of-breed base agent (ECC/wshobson/VoltAgent/dl-ezo) + overlay .tera 20개 + commands 16 + hooks 6 + manifest 배치. genasis-templates crate를 fetch+cache+load 라이브러리로 리팩토링 (reqwest/flate2/tar/dirs). genasis-overlay merger/bootstrap를 AgentStore 인터페이스에 연결. CLI `genasis agents {fetch,status,update,list}` 추가. release-agents.yml CI. agents-pool skeleton (crawl/verify/publish). M14 를 Phase E 로 흡수.
