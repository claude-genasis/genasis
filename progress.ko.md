...
- 2026-05-19: **v0.6.0-alpha.45 — D-132 채팅 thread 전파 + ADR-020 showcase 폴링**. 새 trial 실행 도중 현장 보고로 세 결함이 묶여서 드러남. (1) PM 의 "✅ 배포 완료" wrap-up 이 사람 요청에 thread reply 가 아니라 새 top-level 메시지로 떨어졌고, frontend / devops 는 아예 보고를 안 했음. Root cause: `crates/genasis-cli/src/listen/session.rs:204` `send_user_message` 가 텍스트만 forward — 발신 시 사람의 `post_id` 가 PM 에 도달하기 전에 버려졌다. 그래서 agent overlay 가 `root_id=<human msg id>` 라고 literal 로 적어도 PM 이 치환할 값을 모름. (2) ShowcasePanel iframe 이 devops 의 `genasis push --dir ./app/dist` 성공 + 운영자 served URL 도달 후에도 "building / 채팅 패널에 만들고 싶은 앱을 알려주세요" placeholder 그대로. ShowcasePanel 이 `sim_teams.dev_server_url` (pre-ADR-020 reverse-proxy 컬럼) 만 폴링했고 dev-server API endpoint 가 새 `showcase_pushed_at` 컬럼을 응답에 노출 안 했음. (3) PM overlay (en + ko) 가 alpha.40 의 devops overlay ADR-020 재작성에도 불구하고 옛 v0.5.x `setsid -f sh -c 'npm run dev -- --host 0.0.0.0 --port 5173 …'` + `mcp__trial-app__announce_dev_server_url(...)` dispatch 스크립트 보유 — PM 이 devops 에게 모순된 두 흐름을 동시 시킴. 이 환경 (agent = 사용자 노트북, trial-app = 리모트 운영자 서버) 에선 localhost dev-server 경로가 운영자 측에서 물리적으로 접근 불가. genasis main + agents-pool submodule 동시 fix: (a) `session.rs` `send_user_message(text, post_id)` 가 사람 content 를 `<human_post id="…">…</human_post>` + 명시적 reminder 라인으로 wrap. daemon system prompt 에 "Thread discipline (D-132)" 단락 추가 — PM 이 id 추출 → 모든 `post_message` 에 `root_id` 박고 `Task(subagent_type=…)` prompt 안에도 forward 해서 frontend/devops/qa 가 같은 root 아래 묶임. `listen/mod.rs` caller 가 `InboundEvent::PostCreated` 에서 `post_id` destructure 해서 전달. (b) `agents/overlays/{en,ko}/devops.patch.md.tera` — 모든 `post_message(actor="devops", ...)` 에 `root_id="<human post_id>"` + "Thread 규율" preamble. (c) `agents/overlays/{en,ko}/pm.patch.md.tera` — Step 4 dispatch prompt 가 legacy `npm run dev` + `announce_dev_server_url` 레시피 → ADR-020 `(cd app && npm install && npm run build)` + `genasis push --dir ./app/dist` 으로 재작성. Step 5/6 의 모든 `post_message` 에 `root_id` 추가, "배포 라우팅" 섹션도 같은 흐름으로 동기화. `localhost:5173` 이 리모트 운영자에서 접근 불가 → legacy reverse-proxy 더 이상 동작 안 함 경고 명시. (d) `agents-pool` submodule (commit `9d4f13e`): `/api/trial/team-app/dev-server` GET 이 `showcase_pushed_at` 도 SELECT + 응답에 포함; `ShowcasePanel.tsx` poll loop 가 `dev_server_url` OR `showcase_pushed_at` 중 하나라도 non-null 이면 "ready" 로 flip. 부모 repo 의 submodule pointer 갱신. 게이트: `cargo fmt --all -- --check` clean; `cargo clippy --workspace --all-targets` clean; `cargo test --workspace --tests --exclude genasis-e2e` 276 passed, 0 failed, 5 ignored; `trial-app` `npm run typecheck` clean. D-123 (e2e binary RSS leak) 는 여전히 open. workspace.version `0.6.0-alpha.44` → `0.6.0-alpha.45`.
